...
% !BIB program = biber
 ...
 \begin{document}
	...
	After the inference step is finishes and returns a rewrite (in Minotaurs own IR), the canoncializer again performs transformations on the rewrites.
	...
	In the following we explain each canonicalization-step and show its correctness.
	We show for each canonicalization step that applying it yields an equivalent function that has the same depth as the original function.
	In this case two functions are considered equivalent iff appling them to the same arguments yield an equal output (and changes to memory).
	The depth is kept constant as increasing this has large impact on runtime as shown in original paper \cite{liu24:minotaur}.
	...
	\begin{lstlisting}[language=llvm, escapechar=!]
	input:
